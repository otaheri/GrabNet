...
from models.models import VAEshape
from models.models import HandPoseFC
...
def loss_fn(x, reconstructed_x, mean, log_var):
    ...
    ########## reconstruction loss
    weight = torch.tensor([1, 8], dtype=torch.float32).to(device)
    ...
    ###########
    if reconstructed_x.min()<0 or reconstructed_x.max()>1 or x.min()<0 or x.max()>1:
            pass
    # RCL = F.binary_cross_entropy(reconstructed_x, x, size_average=True)
    # RCL = F.binary_cross_entropy(reconstructed_x, x, reduce=False)
    fn = nn.BCEWithLogitsLoss(reduction='none')
    RCL = fn(reconstructed_x,x)

    weight_ = weight[x.data.view(-1).long()].view_as(x)
    RCL = RCL * weight_
    RCL = RCL.mean()

    # basis_points = torch.from_numpy(training_set.generated_bps.astype(np.float32)).to(device)
    # bps_contact = torch.einsum('ij,jk ->ijk',x,basis_points)
    # bps_contact_hat = torch.einsum('ij,jk ->ijk',reconstructed_x.round(),basis_points)
    #
    # location_loss = torch.mean(torch.pow((bps_contact - bps_contact_hat),2))

    # crit = nn.BCEWithLogitsLoss()
    # kl divergence loss
    KLD = -0.5 * torch.sum(1 + log_var - mean.pow(2) - log_var.exp())
    # KLD = -0.5 * torch.sum(mean.pow(2))

    return 1000*RCL + 1e-3*KLD #+ 400*location_loss


def loss_fnshape(x, reconstructed_x, mean, log_var):

    RCL = torch.mean(torch.pow((reconstructed_x - x),2))
    # kl divergence loss
    KLD = -0.5 * torch.sum(1 + log_var - mean.pow(2) - log_var.exp())
    # KLD = -0.5 * torch.sum(mean.pow(2))

    return RCL + 1e-4*KLD #+ 400*location_loss
...
def loss_fn_Handpose(input, y_hat,Y,vertices, joints,body_model, loss_on=None):
    pose = input[:,-495:]
    contact_verts = input[:,:-495]
    ...
    pose_hat = y_hat
    loss_pose = torch.mean(torch.pow((Y - y_hat), 2))
    loss_verts = torch.tensor(0, dtype=torch.float32)
    loss_joints = torch.tensor(0, dtype=torch.float32)

    if loss_on is not None:
        select_hand_params = range(21, 22) + range(40, 55)
        body_model.use_pca = False
        pose = pose.view(batch_size, -1, 3, 3)
        pose_h = pose_hat.view(batch_size, -1, 3, 3)
        pose[:,select_hand_params] = pose_h
        ...
        vertices_hat, joints_hat = lbs(body_model.shape_components.detach(), pose.contiguous(), body_model.v_template,
                               body_model.shapedirs, body_model.posedirs,
                               body_model.J_regressor, body_model.parents,
                               body_model.lbs_weights, pose2rot=False,
                               dtype=body_model.dtype)
        ...
        if 'joint' in loss_on:
          ...
          loss_pose = loss_pose + loss_hands #+ loss_joints
            ...
            # a = np.isin(vertex_label_contact,np.array(range(26,56))).astype(np.int32)
            # idx = np.where(a==1)[0]
            loss_verts = torch.mean(torch.pow((vertices - vertices_hat),2))
            # loss_pose += 2.5*torch.mean(torch.pow((vertices[:,torch.from_numpy(idx).to(torch.long)] - vertices_hat[:,torch.from_numpy(idx).to(torch.long)]),2))

    return  5e4*loss_verts + 50*loss_pose
...
def global2local_pose(global_pose):
    ...
    return local_pose.view(local_pose.shape[0],-1,3,3)
...
def prepare_input4HandPose(body_model,x,y,num_labels=None, use_pca=True):
    ...
    input_pose = output.full_pose.detach()
    # input_pose = tgm.angle_axis_to_rotation_matrix(input_pose.contiguous().view(-1, 3))[:, :3, :3].contiguous().view(x.shape[0], -1)
    input_pose = tgm.angle_axis_to_rotation_matrix(input_pose.contiguous().view(-1, 3))[:, :3, :3].contiguous().view(x.shape[0], -1,3,3)

    # index_list = range(5005)
    # index_list.pop(5000)
    ...
    select_hand_params =  range(21,22)+range(40, 55)
    # select_hand_params =  range(40, 55)

    # global_pose = local2global_pose(input_pose)

    Y = input_pose[:,select_hand_params]

    # Y[:, 21] = global_pose[:, 21]

    Y = Y.view(x.shape[0], -1)
    y[:, 102:] = y[:, 102:]>0
    input = torch.cat([y[:, 102:], input_pose.view(x.shape[0], -1)], dim=1)

    vertices = output.vertices.detach()
    joints = output.joints.detach()
    # Y = torch.cat([Y, joints[:, 21]],dim=1)
    return input,condition, vertices,joints, X,Y
...
def train_poseHand(model, train_iterator, optimizer, body_model):
    # set the train mode
    model.train()
    ...
    for i, (x, y) in enumerate(train_iterator):
        ...
        input,condition, vertices,joints, X, Y = prepare_input4HandPose(body_model,x,y)

        optimizer.zero_grad()
        y_hat = model(X)
        loss = loss_fn_Handpose(input, y_hat,Y,vertices, joints,body_model, loss_on='vertices')
        loss.backward()
        train_loss += loss.item()
        #
        if i % 100 == 0:
    ...
    return train_loss
def eval_poseHand(test_iterator,model,body_model):
    # set the evaluation mode
    # contact_vis_pose(test_set, model)
    model.eval()
    ...
    with torch.no_grad():
        for i, (x, y) in enumerate(test_iterator):
            ...
            # loss
            loss = loss_fn_Handpose(input, y_hat,Y,vertices, joints,body_model)
            test_loss += loss.item()
            if i % 100 == 0:
                print ('iteration %d'%i)
                print ('loss %.4f'%loss)
...
def trainshape(model, train_iterator, optimizer):
    # set the train mode
    model.train()
    ...
    for i, (x, y) in enumerate(train_iterator):
        ...
        optimizer.zero_grad()
        # forward pass
        reconstructed_input, z_mu, z_var,_ = model(input)
        # loss
        loss = loss_fnshape(input, reconstructed_input, z_mu, z_var)
        # backward pass
        loss.backward()
        train_loss += loss.item()
        #
...
def evalshape(test_iterator,model):
    # set the evaluation mode
    model.eval()
...
def contact_vis_new_bps(test_set,model):
    model.eval()
    from psbody.mesh import Mesh, MeshViewers
    mvs = MeshViewers(shape=[1,1])
    model_path = makepath(os.path.join(result_dir, 'snapshot'))
    for idx in np.random.randint(100, len(test_set), 10):
        ...
        for i in range(1):
            ...
            #     cond_label = cond[...,label]
            #     key = name_to_rgb.keys()
            m.set_vertex_colors(vc=name_to_rgb['red'], vertex_indices=np.where(cond == 1)[1])
            # m.show()
            mvs[0][0].set_static_meshes([m])
            raw_input('hit enter')
...
def contact_vis_pose(test_set,model):
    ...
    model_path = '/ps/scratch/body_hand_object_contact/body_models/models/'
...
def contact_vis_poseHand(test_set,model):
    model.eval()
    from psbody.mesh import Mesh, MeshViewers
    mvs = MeshViewers(shape=[1,3],window_height=2000,window_width=3000)
    ...
    for idx in np.random.randint(0, len(test_set), 10):
        ...
        input,condition, vertices,joints,X,Y = prepare_input4HandPose(body_m,torch.from_numpy(a).view(1,-1),torch.from_numpy(b).view(1,-1))

        y_hat = model(X)

        pose_hat = input[:,-495:].view(1,-1,3,3)
        select_hand_params = range(21, 22) + range(40, 55)
        y_hat = y_hat.view(1, -1, 3, 3)
        pose_hat[:, select_hand_params] = y_hat[:]
...
def pointnet_train():
    ...
    model = models.PointNetDenseCls(k=1)
    model.to(device)
    ...
    test_set = ContactDataSet(contact_parent_path, n_sample_points=n_points, train=False,num_contact_labels=32,training_set=training_set, intent='all')
...
if __name__ == '__main__':
    ...
    model = VAE(encoder_layer_sizes=[5000, 1024, 512, 256], latent_size=32,
                decoder_layer_sizes=[256, 512, 1024, 5000], conditional=True, condition_size=5000)

    # model = VAEshape(encoder_layer_sizes=[5000, 1024, 512, 256], latent_size=128,
    #             decoder_layer_sizes=[256, 512, 1024, 5000])

    # model = HandPoseFC(input_size=5000,layer_sizes=[2048, 1024, 512, 256],output_size=9*16,n_points=n_points)
    model.to(device)
    optimizer = torch.optim.Adam(model.parameters(), lr=lr)
    ...
    # contact_parent_path = '/ps/scratch/body_hand_object_contact/contact_results/14_omid/191001_00158/01_thrshld_15e_6_new_labels'
    # contact_parent_path = '/ps/scratch/body_hand_object_contact/contact_results/16_omid/191001_00158/01_thrshld_15e_6_final'
    ...
    test_set = ContactDataSet(contact_parent_path,contact_expr_id, n_sample_points=n_points, train=False,num_contact_labels=32, intent='all',training_set=training_set)
    test_dataset = data.DataLoader(test_set,drop_last=True, **params)
    ...
    for e in range(N_EPOCHS):
        ...
        # train_loss = train_poseHand(model=model, optimizer=optimizer, train_iterator=train_dataset,body_model=body_model)
        # test_loss = eval_poseHand(model=model, test_iterator=test_dataset,body_model=body_model)
        ...
        # train_loss = trainshape(model=model, optimizer=optimizer, train_iterator=train_dataset)
        # test_loss = evalshape(model=model, test_iterator=test_dataset)
...
# to get the color coded contact on the fingers and the body

# color_id = {'1':np.array([1,0,0]), # RTMB
#             '2':np.array([0,1,0]), # RIDX
#             '3':np.array([0,0,1]), # RMID
#             '4':np.array([1,1,0]), # RRNG
#             '5':np.array([1,0,1]), # RPNK
#             '6':np.array([0,1,1]), # RHND
#             '7':np.array([0.5,0,0]), # LTMB
#             '8':np.array([0,0.5,0]), # LIDX
#             '9':np.array([0,0,0.5]), # LMID
#             '10':np.array([0.5,0.5,0]), # LRNG
#             '11':np.array([0.5,0,0.5]), # LPNK
#             '12':np.array([0,0.5,0.5]), # LHND
#             '13':np.array([1,1,1]) # REST
#              }
#
# vertex_id = {}
# mesh = Mesh(filename = '/ps/project/body_hand_object_contact/data/subject_meshes/male/191001_00158_colored1.ply')
# for item in color_id:
#     finger_vertices_in = np.equal(mesh.vc,color_id[item]).all(axis=1)
#     finger_vertices = np.where(finger_vertices_in==True)
#     vertex_id[item]=finger_vertices
# np.save('/ps/project/body_hand_object_contact/contact_results/color_coded_fingers.npy',)
