...
test_objects = ['elephant', 'mug', 'wineglass']
...
class ContactDataSet(data.Dataset):
    def __init__(self,contact_parent_path, n_sample_points=5000, train=True, intent='all'):
        ...
        for subjectID in self.subjects:
            ...
            seq_names = glob.glob(os.path.join(self.contact_parent_path,subjectID,'*.pkl'))
            for seq_name in seq_names:
                basename = os.path.basename(seq_name)
                object_name = basename.split('_')[0]
                if 'all' in intent:
    ...
    def data_preprocessing(self,based_on='object'):
        ...
        self.generated_bps = bps.generate_bps(n_points=self.n_points, radius=1.1)
            ...
            for seq_name in iter_on[object_name]:
                ...
                if object_name not in self.object_pts:
                    ...
                    if False:
                        ...
                        obj_pts_n, mean, scale = bpsn.unit_normalize_batch(object_fullpts.reshape(1, -1, 3), return_scalers=True)
                        object_pts , object_pts_choice = bps.convert_to_bps(obj_pts_n.reshape(1, -1, 3), self.generated_bps, return_deltas=False)
                        self.object_pts[object_name] = object_pts
                ...
                input = object_data
                output = subject_data
                output_contact = np.concatenate([contact_frames_object, contact_frames_body], axis=1).astype(np.bool)
        ...
        self.list_output_contact_data = np.concatenate(output_contact_data).astype(np.bool)
